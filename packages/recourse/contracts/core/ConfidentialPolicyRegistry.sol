...
contract ConfidentialPolicyRegistry is IConfidentialPolicyRegistry, TestnetCoreBase {
    ...
    function registerPolicy(address policy_) external onlyOwner returns (uint256 policyId) {
        ...
        if (!ERC165Checker.supportsInterface(policy_, type(IConfidentialUnderwriterPolicy).interfaceId)) {
