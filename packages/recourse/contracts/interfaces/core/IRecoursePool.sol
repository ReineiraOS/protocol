...
/// @title IRecoursePool — Per-pool liquidity container (plain variant)
/// @notice Deployed by PoolFactory. Holds three roles per whitepaper §7.2:
...
///         LPs stake into the pool; CoverageManager calls payClaim/receivePremium on
///         dispute resolution. For the confidential variant, see IConfidentialRecoursePool.
interface IRecoursePool is ICore, IRecoursePoolEvents {
    ...
    /// @notice Returns the ERC20 token this pool accepts
    ...
    /// @notice Adds an IUnderwriterPolicy to the pool's allowed set (Creator only — §7.2)
    /// @param policy The policy contract address to allow
    function addPolicy(address policy) external;

    /// @notice Removes an IUnderwriterPolicy from the pool's allowed set (Creator only — §7.2)
    ...
    /// @notice Stakes amount into this pool
    ...
    /// @return actualPayout The actual amount transferred (capped by pool liquidity)
    function payClaim(uint256 coverageId, uint256 amount) external returns (uint256 actualPayout);
    ...
    function receivePremium(uint256 coverageId, uint256 premium) external;

    /// @notice Returns the amount for a given stake position
    /// @param stakeId The stake position to query
    function stakedAmount(uint256 stakeId) external view returns (uint256);

    /// @notice Returns the total liquidity available in the pool
    function totalLiquidity() external view returns (uint256);

    /// @notice Returns the rewards earned by a stake position
    /// @param stakeId The stake position to query
    ...
    function claimPremiums(uint256 amount) external;
}
