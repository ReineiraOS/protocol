...
/// @title IConfidentialCoverageManager — Coverage purchase and dispute hub (FHE variant)
/// @notice Buyers purchase coverage permissionlessly against any open ConfidentialRecoursePool.
///         Private (closed) pools require an EIP-712 voucher signed by the pool Manager
///         per whitepaper §7.10 (voucher is plaintext; only the coverage holder identity
///         and amount remain encrypted). The pool's IConfidentialUnderwriterPolicy gates
///         risk evaluation and dispute resolution.
interface IConfidentialCoverageManager is ICore, ICoverageManagerEvents {
    ...
    ///         registers coverage.
    /// @param encryptedHolder Encrypted address of the coverage holder
    /// @param pool The ConfidentialRecoursePool backing this coverage
    /// @param policy The IConfidentialUnderwriterPolicy to evaluate risk
    ...
    /// @param policyData Policy-specific data passed to policy.onPolicySet()
    /// @param riskProof Proof data passed to policy.evaluateRisk()
    ...
    /// @notice Returns the confidential escrow contract address
    function escrow() external view returns (address);

    /// @notice Sets the escrow contract address
    /// @dev Set via setter (not initialize) to resolve circular dependency with ConfidentialEscrow
    ...
    /// @notice Returns the ConfidentialPoolFactory used to validate pools
