...
/// @title IPoolFactory — Central registry for recourse pools (plain variant)
/// @notice Anyone may call createPool() to deploy a new RecoursePool. The caller
///         becomes the immutable Pool Creator; they choose the initial Pool Manager
///         (defaults to the creator), an optional Guardian, and whether the pool is
///         open (anyone may purchase coverage) or private (purchase requires a
///         Manager-signed EIP-712 voucher). See whitepaper §7.2 / §7.10.
///         For the confidential variant, see IConfidentialPoolFactory.
interface IPoolFactory is ICore, IPoolFactoryEvents {
    ...
    /// @notice Deploys a new RecoursePool with the caller as the immutable Pool Creator
    /// @param paymentToken The ERC20 token the pool accepts (must be in allowlist)
    ...
    /// @return pool Address of the deployed RecoursePool
    function createPool(
    ...
    /// @param poolId The pool identifier to look up
