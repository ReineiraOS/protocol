...
/// Deploys a fresh verdict-backed demo stack and mints N unclaimed coverages held
/// by the client — the queue the console's ChainFundingSource disputes per "Simulate".
contract DemoProvisionCoverages is Script {
    ...
    function run() external {
        ...
        uint256 n = vm.envOr("COVERAGE_COUNT", uint256(5));
        address tf = vm.envOr("TRUSTED_FORWARDER", address(0));

        uint256 stakeAmount = COVERAGE * n + COVERAGE;
        ...
        IERC20(USDC).approve(poolAddr, stakeAmount);
        ...
        for (uint256 i = 0; i < n; i++) {
            cm.purchaseCoverage(client, poolAddr, address(policy), ESCROW_ID, COVERAGE, block.timestamp + 30 days, "", "");
        }
        ...
        console.log("COVERAGE_COUNT=%s", n);
